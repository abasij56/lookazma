(function () {
	...
	function showResult(productName, text, isError, productId, currentDescription, categoryUrl) {
		...
		var refineBtn = document.getElementById('ai-product-desc-refine-btn');
		...
		if (isError) {
			...
			lastResult.productName = productName || lastResult.productName || '';
			...
			if (refineBtn) {
				...
				refineBtn.classList.remove('is-loading', 'is-error');
			}
			setSaveStatus('');
	...
	function handleGenerateClick(event) {
		...
		if (!btn || btn.id === 'ai-product-desc-refine-btn') {
			return;
		}
	...
	function handleRefineClick(event) {
		var btn = event.target.closest
			? event.target.closest('#ai-product-desc-refine-btn')
			: null;
		...
		if (!lastResult.productId || !lastResult.description) {
			window.alert(
				(cfg.i18n && cfg.i18n.refineError) || 'ابتدا توضیحات را بسازید.'
			);
			return;
		}

		if (!cfg.ajaxUrl || !cfg.refineAction || !cfg.nonce) {
			window.alert(
				(cfg.i18n && cfg.i18n.refineError) ||
					'پیکربندی اسکریپت بارگذاری نشده است. صفحه را رفرش کنید.'
			);
			return;
		}
		...
		btn.classList.add('is-loading');
		btn.classList.remove('is-error');
		btn.textContent = (cfg.i18n && cfg.i18n.refining) || 'در حال ویرایش...';
		setSaveStatus('');

		var formData = new FormData();
		formData.append('action', cfg.refineAction);
		formData.append('nonce', cfg.nonce);
		...
		fetch(cfg.ajaxUrl, {
			...
			.then(function (result) {
				...
				if (!result.payload || !result.payload.success) {
					...
					var message =
						(result.payload &&
							...
							result.payload.data.message) ||
						(cfg.i18n && cfg.i18n.refineError) ||
						'خطا در ویرایش بر اساس برند و گرید.';
				...
				var data = result.payload.data || {};
				showResult(
					data.product_name || lastResult.productName || '',
					data.description || '',
					false,
					data.product_id || lastResult.productId,
					undefined,
					data.category_url || lastResult.categoryUrl || ''
				);
			})
			.catch(function () {
				btn.disabled = false;
	...
	document.addEventListener('click', handleRefineClick, true);
