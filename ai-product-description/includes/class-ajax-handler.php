...
final class AI_Product_Desc_Ajax {
	...
	public const REFINE_ACTION      = 'ai_product_desc_refine_brand_grade';
	...
	public static function init(): void {
		...
		add_action( 'wp_ajax_' . self::REFINE_ACTION, array( __CLASS__, 'refine_brand_grade' ) );
	...
	/**
	 * Rewrite generated description based on brand and grade (example.txt prompt).
	 */
	public static function refine_brand_grade(): void {
		self::guard_request();

		$product    = self::get_master_product_from_request();
		$product_id = $product->get_id();
		$raw_text   = isset( $_POST['description'] ) ? wp_unslash( $_POST['description'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- HTML from AI preview.
		$raw_text   = is_string( $raw_text ) ? trim( $raw_text ) : '';

		if ( '' === $raw_text ) {
			wp_send_json_error(
				array(
					'message' => __( 'متن توضیحات برای ویرایش خالی است.', 'ai-product-description' ),
				),
				400
			);
		}
		...
		$result = AI_Product_Desc_Client::refine_description_brand_grade( $product_data );
