<?php
/**
 * Light post category archive — e.g. /category/article/.
 *
 * Reuses magazine layout (sidebar products + post cards) and shared chrome.
 *
...
/**
 * Switches blog category archives to the Timber light article layout.
 */
final class Hello_Elementor_Child_Custom_Post_Category_Archive {
	...
	public static function init(): void {
		...
		add_action( 'wp_ajax_lk_post_category_load_posts', array( __CLASS__, 'ajax_load_posts' ) );
		add_action( 'wp_ajax_nopriv_lk_post_category_load_posts', array( __CLASS__, 'ajax_load_posts' ) );
	 ...
	 */
	public static function get_term(): ?WP_Term {
	...
	public static function get_template_path(): string {
		return HELLO_ELEMENTOR_CHILD_PATH . 'category-light.php';
	}

	/**
	 * Hard takeover before Elementor theme templates.
	 */
	...
	public static function disable_elementor_locations( bool $need_override, string $location ): bool {
		...
		$blocked = array( 'header', 'footer', 'archive', 'single', 'popup' );
	...
	/**
	 * Strip Elementor Theme Builder chrome hooks.
	 */
	...
	public static function filter_breadcrumb( array $crumbs ): array {
		...
		return array(
			array( __( 'خانه', 'hello-elementor-child' ), home_url( '/' ) ),
			array( $term->name, '' ),
		);
	}
		...
		if ( file_exists( $article_js ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'action'  => 'lk_post_category_load_posts',
					'nonce'   => wp_create_nonce( 'lk_post_category_posts' ),
					'pageId'  => 0,
					'catId'   => $term ? (int) $term->term_id : 0,
				)
			);
		}
	}
	...
	public static function body_class( array $classes ): array {
		...
		$classes[] = 'lk-light-post-category';
	...
	/**
	 * Posts + pagination for a category archive.
	 *
	 * @return array<string, mixed>
	 */
	public static function get_listing_data( int $cat_id, int $paged ): array {
		$paged  = max( 1, $paged );
		$cat_id = max( 0, $cat_id );
		$query  = new WP_Query(
			array(
		...
		$pagination = '';
		$term_link  = $cat_id > 0 ? get_category_link( $cat_id ) : '';
		if ( (int) $query->max_num_pages > 1 && is_string( $term_link ) && '' !== $term_link ) {
		...
		return array(
			...
			'paged'           => $paged,
			'url'             => self::get_paged_url( $cat_id, $paged ),
		);
	}

	/**
	 * Public URL for a category listing page.
	 */
	public static function get_paged_url( int $cat_id, int $paged ): string {
		$permalink = $cat_id > 0 ? (string) get_category_link( $cat_id ) : '';
		if ( '' === $permalink ) {
			return '';
		}
		if ( $paged < 2 ) {
	...
	/**
	 * AJAX category pagination.
	 */
	public static function ajax_load_posts(): void {
		check_ajax_referer( 'lk_post_category_posts', 'nonce' );
		...
		$listing = self::get_listing_data( $cat_id, $paged );
		$html    = '';
	...
	/**
	 * Page content for Twig (same shape as light-article).
	 *
	 * @return array<string, mixed>
	 */
	public static function get_page_content(): array {
		$term  = self::get_term();
		$title = $term ? $term->name : __( 'مقالات', 'hello-elementor-child' );
