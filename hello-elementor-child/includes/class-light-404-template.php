<?php
/**
 * Light 404 — chemistry-themed native chrome (always on for is_404).
 *
...
/**
 * Custom 404 with shared Lookazma header/footer.
 */
final class Hello_Elementor_Child_Light_404_Template {
	...
	public static function get_template_path(): string {
		return HELLO_ELEMENTOR_CHILD_PATH . '404-light.php';
	}

	/**
	 * Take over early on 404.
	 */
	...
	public static function disable_elementor_locations( bool $need_override, string $location ): bool {
		...
		if ( in_array( $location, array( 'header', 'footer', 'single', 'archive', 'popup', '404' ), true ) ) {
			return false;
		}
		return $need_override;
	}
	...
	public static function enqueue_assets(): void {
		...
		if ( file_exists( $css ) ) {
			wp_enqueue_style(
				'lk-light-404',
				HELLO_ELEMENTOR_CHILD_URI . 'assets/css/light-404.css',
	...
	public static function get_page_content(): array {
		...
		return array(
			'code'         => '404',
			'formula'      => 'C₀H₀',
			'headline'     => __( 'واکنش ناقص ماند', 'hello-elementor-child' ),
			'lead'         => __( 'آزمایش ما این آدرس را شناسایی نکرد — شاید لینک قدیمی باشد یا مسیر اشتباه تایپ شده. بیایید مسیر تازه‌ای انتخاب کنیم.', 'hello-elementor-child' ),
			'home_url'     => home_url( '/' ),
			'home_label'   => __( 'صفحه اصلی', 'hello-elementor-child' ),
			'shop_url'     => $shop,
			'shop_label'   => __( 'محصولات', 'hello-elementor-child' ),
			'hint'         => __( 'جستجو در هدر همیشه در دسترس است', 'hello-elementor-child' ),
		);
	}

	/**
	 * Always use the live shop archive URL (فروشگاه), never trashed/query fallbacks.
	 */
